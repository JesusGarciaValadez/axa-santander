...
( function ( $, window, document, undefined ) {
    ...
    var _AXAEncuestas    = window._AXAEncuestas,
    // Use the correct document accordingly with window argument (sandbox)
    document    = window.document,
    location    = window.location,
    navigator   = window.navigator,
    // Map over AXAEncuestas in case of overwrite
    _AXAEncuestas    = window.AXAEncuestas;
    // Define a local copy of AXAEncuestas
    AXAEncuestas = function() {
        if ( !( this instanceof AXAEncuestas ) ) {
            // The AXAEncuestas object is actually just the init constructor 'enhanced'
            return new AXAEncuestas.fn.init();
        }
        return AXAEncuestas.fn.init();
    };

    AXAEncuestas.overlay;
    AXAEncuestas.closer;
    AXAEncuestas.radio;
    AXAEncuestas.tool;
    AXAEncuestas.columnWidth = 188;

    //  Object prototyping
    AXAEncuestas.fn = AXAEncuestas.prototype = {
        /**
         *
        ...
        init:                   function ( ) {
            //AXAEncuestas.obtainActualDocument();
            if( $( window ).innerWidth() <= 568 ) {
                AXAEncuestas.columnWidth = 50;
            } else if ( $( window ).innerWidth() >= 569 && $( window ).innerWidth() <= 1918 ) {
                AXAEncuestas.columnWidth = 22;
            } else {
                AXAEncuestas.columnWidth = 347;
            }
        },
        /**
         *
        ...
        anchorMenu:             function ( selectorToApply, offsetTop, classToFix, classToDeFix ) {
            AXAEncuestas.tool = (window.pageYOffset !== undefined) ? window.pageYOffset : (document.documentElement || document.body.parentNode || document.body).scrollTop;
            ...
            if ( AXAEncuestas.tool >= _offsetTop ) {
        ...
        closeAlert:             function ( ) {
            ...
            if ( typeof( AXAEncuestas.closer ) !== 'undefined' ){
                AXAEncuestas.overlay.close();
            } else {
        ...
        validateContactForm:    function ( _rules, _messages, _submitFunction, _invalidFunction ) {
            var formActive = $( 'form' ).validate( {
                ...
                errorPlacement: function( error, element ) {
                    element.parents( '.center' ).prepend( error );
                },
                //debug:true,
                rules: _rules,
                messages: _messages,
    ...
    // Give the init function the AXAEncuestas prototype for later instantiation
    AXAEncuestas.fn.init.prototype = AXAEncuestas.fn;

    AXAEncuestas = AXAEncuestas.fn;

    // Expose AXAEncuestas to the global object
    window.AXAEncuestas  = AXAEncuestas;
