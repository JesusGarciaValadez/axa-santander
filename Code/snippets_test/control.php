...
switch ($action) {
    ...
    case 'password':
        header('Content-Type: application/json');
        $usuario = new Usuarios($dbh);
        $success = $usuario->resetPassword($_POST);
        break;
    ...
    case 'actualizarPuntaje':
        header('Content-Type: application/json');
        $puntaje = new ActualizarPuntajes( $dbh );
        $success = $puntaje->actualizarPuntajeVendedores( $_POST );
        $success = json_encode( $success );
        break;
    case 'obtenerReporte':
    ...
    case 'descargarReporte':
        $descarga = new DescargarOrdenes( $dbh );
        $success = $descarga->descargarRepote( $_SESSION['user']['ordenes'] );
